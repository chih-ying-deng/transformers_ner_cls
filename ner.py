...
parser.add_argument('--dset', type=str, default='data/symptom_large', help='name of dataset')
parser.add_argument('--seed', type=int, default=123, help='seed')
parser.add_argument('--lr', type=float, default=6e-5, help='learning rate')
parser.add_argument('--decay', type=float, default=0.02, help='weight decay ate')
...
MODEL_CLASSES = {
    'bert': (BertForTokenClassification, BertTokenizer, 'bert-base-cased'),
    'xlnet': (XLNetForTokenClassification, XLNetTokenizer, 'xlnet-base-cased'),
    'clinicalxlnet': (XLNetForTokenClassification, XLNetTokenizer, 'xlnet-base-cased'),
    'roberta': (RobertaForTokenClassification, RobertaTokenizer, 'roberta-base'),
    'xlm-roberta': (XLMRobertaForTokenClassification, XLMRobertaTokenizer, 'xlm-roberta-base'),
    'camembert': (CamembertForTokenClassification, CamembertTokenizer, 'camembert-base'),
    'distilbert': (DistilBertForTokenClassification, DistilBertTokenizer, 'distilbert-base-cased'),
    'electra': (ElectraForTokenClassification, ElectraTokenizer, 'google/electra-small-discriminator')
}
...
class TransformerNER():
    ...
    def prepare_data(self):
        ...
        for mode in ['train', 'valid', 'test']:
            ...
            tokenized_sentences, tokenized_labels, self.tokenized_idx, self.tokenized_token = [], [], [], []
            for sent, label, id in zip(sentences, labels, idx):
                ...
                self.tokenized_idx.append(tokenized_id)
                self.tokenized_token.append(tokenized_tok)
    ...
    def format_tags(self, predictions, true_labels):
        """
        convert ids to original labels and create formated output prediction
        """
        pred_tags, label_tags, out = [], [], []
        for prediction, true_label, token, id in zip(predictions, true_labels, self.tokenized_token, self.tokenized_idx):
    ...
    def trainer(self, parameterization, weight=None):
        ...
        for epoch in range(self.n_epochs):
            ...
            pred_tags, train_tags, out = self.format_tags(predictions, true_labels)
            ...
            pred_tags, valid_tags, out = self.format_tags(predictions, true_labels)
            ...
            # format output and calculate metrics
            pred_tags, test_tags, out = self.format_tags(predictions, true_labels)
